...
async def test_select(live_c8y: CumulocityClient, safe_create):
    ...
    all_roles = await live_c8y.user_groups.get_all(limit=None, page_size=100)
    ...
    for role in await live_c8y.user_groups.get_all(limit=None, username=username, page_size=100):
        assert role.id in selected_ids

    # client-side filter on all roles
    filtered_1 = await live_c8y.user_groups.get_all(limit=None, include="name contains Global")
    ...
    assigned = await live_c8y.user_groups.get_all(limit=None, username=username)
    filtered_1 = await live_c8y.user_groups.get_all(limit=None, username=username, include="name contains a")
...
async def test_updating_users(live_c8y: CumulocityClient, safe_create):
    ...
    groups = await live_c8y.user_groups.get_all(limit=None, username=current_username)
